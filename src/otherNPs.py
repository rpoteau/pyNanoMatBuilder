...
class fcctpt:
    ...
    def __init__(self,
        ...
        if not noOutput: vID.centerTitle(f"fcc triangular platelet with {nLayer*2+1} remaining shells, made from a trigonal bipyramid with {nLayerTd} shells per pyramid")
    ...
    def coords(self,noOutput):
        ...
        if not noOutput: print(f"signed distance between truncation plane 1 and origin = {pyNMBu.Pt2planeSignedDistance(truncationPlane1,[0,0,0]):.2f}")
