...
class fcctbp:
    ...
    heightOfBiPyramid = 2*heightOfPyramid

    def __init__(self,
                 element: str='Au',
                 Rnn: float=2.7,
                 nLayerTd: int=1):
        self.Td = pNP.regfccTd(element,Rnn,nLayerTd)
        self.element = element
        self.Rnn = Rnn
        self.nLayerTd = int(nLayerTd-1)
    ...
    def __str__(self):
        return(f"Regular fcc double tetrahedron with {self.nLayer+1} layer(s) and Rnn = {self.Rnn}")
    ...
    def coords(self):
        ...
        nAtoms = aseObject.get_number_of_atoms()
