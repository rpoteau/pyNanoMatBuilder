...
class Crystal:
    ...
    def makeWulff(self,noOutput):
        ...
        if self.eSurfacesWulff is not None: eSurf = []
        for i,p in enumerate(self.surfacesWulff):
            pNorm = pNMBu.normV(p)
            if self.symWulff: 
                symP = self.sg.equivalent_lattice_points(pNorm)
                print(symP,len(symP),len(symP[0]))
                trPlanes.append(symP)
                if self.eSurfacesWulff is None: sizes.append(len(symP)*self.sizesWulff[i])
                if self.eSurfacesWulff is not None: eSurf.append(len(symP)*self.eSurfacesWulff[i])
            else:
                trPlanes.append(pNorm)
        ...
        if self.eSurfacesWulff is None: 
            ...
            sizes = -np.array(self.sizesWulff)*10
        if self.eSurfacesWulff is not None: eSurf = np.array(eSurf)
        print(f"{trPlanes = }\n")
        print(f"{sizes = }")
        print(f"{eSurf = }")
